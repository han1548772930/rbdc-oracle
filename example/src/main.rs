...
async fn main() {
    ...
    //remode old data
    let deleted = StudentProfile::delete_all(&mut rb)
        .await
    ...
    let update_result = Student::update_by_column(&mut tx, &update_stu, "id_card")
        .await
        .expect("update failed");
    assert_eq!(update_result.rows_affected, 1);

    //delete
    let delete_result = Student::delete_by_column(&mut tx, "id_card", "2500000000")
        .await
        .expect("delete failed");
    assert_eq!(delete_result.rows_affected, 1);

    //select in transaction
    let selected = Student::select_by_column(&mut tx, "id_card", "2500000000")
        .await
    ...
    let selected = Student::select_by_column(&mut rb, "id_card", "2800000000")
        .await
        .expect("select failed");
    assert_eq!(selected.len(), 1);

    //remove new student
    Student::delete_by_column(&mut rb, "id_card", "2800000000")
        .await
        .expect("delete failed");
