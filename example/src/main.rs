...
pub struct StudentProfile {
    pub id_card: i64,
    pub photo: Option<rbatis::rbdc::bytes::Bytes>,
...
async fn main() {
    ...
    rb.init_option::<OracleDriver, OracleConnectOptions, DefaultPool>(
        ...
        OracleConnectOptions {
            username: "HDDZ".to_string(),
            password: "HDDZ".to_string(),
            connect_string: "//10.66.66.250:1521/HDDZ/".to_string(),
        },
    )
    ...
    // rb.exec("select get_sysid('mes_map_detail') id from dual",vec![]).await.unwrap();
    //use "last_insert_id" to receive the params
    let res = rb
        .exec("select get_sysid('mes_map_detail') id from dual", vec![])
        .await
        .unwrap();
